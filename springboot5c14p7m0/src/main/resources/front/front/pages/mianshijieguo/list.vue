			...
			<view :style='{"minHeight":"100%","padding":"0 20rpx","flexWrap":"wrap","background":"#E9E9E9","display":"flex","width":"100%","position":"relative","height":"auto"}'>
					...
					<view class="list" :style='{"width":"100%","padding":"0","margin":"0","borderRadius":"20rpx","background":"none","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","height":"auto"}'>
								...
								<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx","margin":"0","overflow":"hidden","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>{{product.mianshijieguo}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('mianshijieguo','修改')) || (!userid && isAuthFront('mianshijieguo','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('mianshijieguo','删除')) || (!userid && isAuthFront('mianshijieguo','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('mianshijieguo','修改')) || (!userid && isAuthFront('mianshijieguo','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('mianshijieguo','删除')) || (!userid && isAuthFront('mianshijieguo','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>面试结果</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="mianshijieguoChange(item)" v-for="(item,index) in mianshijieguoOptions" :key="index" class="screenTab" :class="searchForm.mianshijieguo==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			mianshijieguoChange(e){
				this.searchForm.mianshijieguo = e
				this.$forceUpdate()
			},
			screenReset(){
				this.searchForm = {}
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.mianshijieguo){
					params['mianshijieguo'] = '%' + this.searchForm.mianshijieguo + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`mianshijieguo`, params);
                } else {
                    res = await this.$api.list(`mianshijieguo`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('mianshijieguo', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.mianshijieguo){
					searchForm['mianshijieguo'] = '%' + this.searchForm.mianshijieguo + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`mianshijieguo`, searchForm);
                } else {
                    res = await this.$api.list(`mianshijieguo`, searchForm);
                }
				// 如果是第一页数据置空
